...
void ExtractFF(){
  ...
  SashFile::HandlerC *han = new SashFile::HandlerC("CT3_FlatFieldCoeff"); 
  ...
  Sash::DataSet *ds = han->GetDataSet("CT3_FlatFieldCoeff"); 
  ...
  Sash::Telescope *CT3 = fHess->GetTel(3);

  const HDCalibration::TelescopeFFInfos *CT3_ff = CT3->Get< HDCalibration::TelescopeFFInfos > ("FFInfos");
 
  CT3_ff->DisplayFFCharge();
//////////////////////////////////////////////////////////////////////////////////////////////
//
//	Fix me if you can!!!!
//
...
//
  ...
  for (Sash::Pointer<Sash::Pixel> pix = CT3->beginPixel(); pix != CT3->endPixel(); ++pix) 
    ...
    char *hand = "hand";
    char *ct = "CT";
    const HDCalibration::PixelFFInfos *pixff = CT3_ff->GetMonitor(pix);
    
  ////////////  if (PixId == 152) continue;
	...
	else if (LoEntries==0 || LoEntries < 300)
	{
	meanLo = -1.;	 
    	rmsLo  = -1.;
	coefLo = .0;
